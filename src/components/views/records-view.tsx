...
import { hasRichTextContent, RichTextEditor, RichTextViewer } from "../rich-text-editor";
...
function JournalModal({ data, onClose, onAdd, entry }: { data: RenovationData; onClose: () => void; onAdd: (entry: JournalEntry) => void; entry?: JournalEntry }) {
  ...
  const isEdit = !!entry;
  return <Modal title={isEdit ? "编辑施工日志" : "记录今天的现场情况"} onClose={onClose}><form className="form-grid" onSubmit={(event) => { event.preventDefault(); if (!title.trim() || !hasRichTextContent(summary) || !date) return; onAdd({ id: entry?.id ?? crypto.randomUUID(), date, title: title.trim(), summary, stageId, photoCount: attachments.length, attachments }); }}><label className="field"><span>日期</span><input type="date" required value={date} onChange={(event) => setDate(event.target.value)} /></label><label className="field"><span>所属阶段（可选）</span><select value={stageId} onChange={(event) => setStageId(event.target.value)}><option value="">暂不关联阶段</option>{data.stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.name}</option>)}</select></label><label className="field field-full"><span>日志标题</span><input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder="例如：水电打压验收" required /></label><div className="field field-full"><span>现场记录</span><RichTextEditor value={summary} onChange={setSummary} placeholder="今天完成了什么，发现了什么，下一步要做什么" /></div><div className="field-full"><ImageAttachments value={attachments} onChange={setAttachments} label="现场照片（可选）" max={9} /></div><div className="form-actions field-full"><button className="secondary-button" type="button" onClick={onClose}>取消</button><button className="primary-button" type="submit"><ClipboardText size={17} /> {isEdit ? "更新日志" : "保存日志"}</button></div></form></Modal>;
}
